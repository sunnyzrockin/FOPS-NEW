    ...
    for (const region of REGION_ANCHORS) {
      ...
      for (let i = 0; i < region.count; i++) {
        ...
        for (const [fuel, base] of Object.entries(FUEL_BASE_CENTS)) {
          ...
          // All math stays in "cents" (integer). Base values are already
          // in cents (e.g. ULP91 base = 189c = $1.89).
          const stationDev = Math.floor((rand() - 0.5) * 12);
          const bucketJitter = ((minuteBucket + idx) % 7) - 3;
          const price_cents = Math.max(70, Math.round(base + stationDev + bucketJitter));
          prices.push({
            station_id,
            fuel_type: fuel,
            price_cents,
