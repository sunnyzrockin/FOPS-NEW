...
// Brand names from QLD FPM are inconsistent ("7 Eleven" vs "7-Eleven",
// "Reddy Express" vs "Reddy-Express", "EG  Fuel" with double-space, …).
// Normalise so the filter dropdown shows a single entry per chain.
function normaliseBrand(raw) {
  ...
  const trimmed = String(raw)
    ...
    .trim();
  // Canonical short-forms for the chains people recognise.
  ...
  async _loadBrandMap() {
    ...
    try {
      ...
      for (const b of arr) {
        ...
        if (id != null) map.set(Number(id), normaliseBrand(String(name)));
      }
      return map;
    } catch (err) {
