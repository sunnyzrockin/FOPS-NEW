...
export async function sendInviteEmail({ to, inviterName, role, token, siteName }) {
  // The invite acceptance page lives at /accept-invite and reads the token
  // from the `token` query param (it pre-existed the Phase 2 work). Keep the
  // URL shape stable so older invite emails in inboxes continue to work.
