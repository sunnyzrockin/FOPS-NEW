...
export function generateFuelPriceData(sites, users) {
  ...
  sites.forEach((site, siteIndex) => {
    ...
    for (let i = 0; i < numCompetitors; i++) {
      // Generate coordinates within 1-5km radius of the site
      const distanceKm = 0.5 + Math.random() * 4.5;
      const angle = Math.random() * 2 * Math.PI;
      const latOffset = (distanceKm / 111) * Math.cos(angle); // 111km per degree
