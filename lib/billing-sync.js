...
export async function syncQuantityForOwner(ownerId, { prorate = true } = {}) {
  ...
  if (!hasStripe || !hasPerSite) {
    const reason = !hasStripe
      ? 'stripe_secret_missing'
      : 'per_site_price_missing';
    ...
    return {
      ...
      reason,
      env: {
        STRIPE_SECRET_KEY: hasStripe ? 'set' : 'MISSING',
        STRIPE_PRICE_PER_SITE: hasPerSite ? 'set' : 'MISSING',
        STRIPE_PRICE_BASE: hasBase ? 'set' : 'MISSING',
      },
    };
  }
