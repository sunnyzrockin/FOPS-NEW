...
export async function handleGetDashboardStats(request) {
  try {
    ...
    try {
      const [todayReports, pendingAll, varianceAll] = await Promise.all([
        ...
        db.from('shift_reports')
          ...
          .in('site_id', siteIdArray)
          .eq('status', 'pending'),
        // Variance alert proxy: reports flagging drive-offs (loss). A
        // dedicated `is_variance` column can replace this later without
        // changing the client.
        db.from('shift_reports')
          .select('id', { count: 'exact', head: true })
          .in('site_id', siteIdArray)
      ...
      submittedToday = new Set((todayReports.data || []).map((r) => r.site_id)).size;
