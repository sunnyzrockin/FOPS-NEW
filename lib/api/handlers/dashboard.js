...
export async function handleGetDailyRollups(request) {
  try {
    ...
    (reports || []).forEach(report => {
      ...
      if (!rollups[key]) {
        rollups[key] = {
          ...
          banking_value: 0,
          shift_count: 0,
          pending_count: 0,   // Bug #3: per-rollup status counters
          reviewed_count: 0,
          flagged_shift_count: 0, // P1: count of shifts that failed reconciliation
          shifts: [],         // Bug #3: per-shift array consumed by DailyRollupRow
        };
      ...
      // Minor finding #2: banking_value is computed downstream from the
      // site's banking formula (see formula_results block below). Not
      // accumulated here because shift_reports has no banking column.
      rollups[key].shift_count   += 1;
      // Bug #3: status counters per rollup
      if (report.status === 'pending' || report.status === 'submitted') {
        rollups[key].pending_count += 1;
      } else if (report.status === 'reviewed' || report.status === 'approved') {
        rollups[key].reviewed_count += 1;
      }
      // Bug #3: per-shift summary row used by DailyRollupRow expansion.
      rollups[key].shifts.push({
        id: report.id,
        shift_type: report.shift_type || 'Shift',
    ...
    // Bug #3: enrich rollups with site_name and sort each rollup's
    // shifts[] in the conventional Morning → Afternoon → Night order so
    // the expanded section renders predictably.
    {
      const uniqueSiteIds = Array.from(new Set(Object.values(rollups).map((r) => r.site_id)));
      if (uniqueSiteIds.length > 0) {
        const { data: siteRows } = await (supabaseAdmin || supabase)
          .from('sites').select('id, name').in('id', uniqueSiteIds);
        const nameById = new Map((siteRows || []).map((s) => [s.id, s.name]));
        const shiftOrder = { Morning: 1, Afternoon: 2, Night: 3 };
        for (const key in rollups) {
          rollups[key].site_name = nameById.get(rollups[key].site_id) || rollups[key].site_id;
          rollups[key].shifts.sort((a, b) =>
            (shiftOrder[a.shift_type] || 99) - (shiftOrder[b.shift_type] || 99)
          );
        }
      }
    }
    ...
    for (const key in rollups) {
      ...
      if (formulas && formulas.length > 0) {
        ...
        // Minor finding #2: banking_value was always 0 because shift_reports
        // has no `banking_value` / `banking` column to read from. The real
        // canonical banking number is the result of the per-site "Banking"
        // formula. Pick the formula whose name or result_label includes
        // "banking" (case-insensitive); fall back to the first formula so
        // the daily row never reads blank when there's any formula at all.
        const bankingFormula = rollup.formula_results.find((fr) =>
          /banking/i.test(fr.formula_name) || /banking/i.test(fr.result_label)
        ) || rollup.formula_results[0];
        if (bankingFormula) {
          rollup.banking_value = Number(bankingFormula.result_value) || 0;
        }
      }
    }
    
    // Bug #9: sort by date desc (most recent first), then by site_name asc
    // within a single date. Without this the rollups come out in whatever
    // order the reports were fetched, which is effectively random and
    // makes the Daily Summaries list confusing.
    const rollupArray = Object.values(rollups).sort((a, b) => {
      if (a.date < b.date) return 1;
      if (a.date > b.date) return -1;
      return (a.site_name || '').localeCompare(b.site_name || '');
    });

    return NextResponse.json(rollupArray, { headers: corsHeaders });
  } catch (error) {
...
export async function handleGetDashboardStats(request) {
  try {
    ...
    try {
      const [todayReports, pendingAll, redTanksToday, sitesMeta] = await Promise.all([
        ...
        db.from('shift_reports')
          ...
          .in('site_id', siteIdArray)
          .eq('status', 'pending'),
        // Wet-stock Tier 1: count tanks with status='red' for TODAY across
        // the in-scope sites. This replaces the legacy drive_offs-based
        // proxy with a real fuel-loss signal. If the tank_reconciliation
        // table is missing (pre-migration), the catch falls through and
        // varianceAlerts stays 0.
        db.from('tank_reconciliation')
          .select('id', { count: 'exact', head: true })
          .in('site_id', siteIdArray)
          .eq('date', todayISO)
          .eq('status', 'red'),
      ...
      varianceAlerts = redTanksToday.count || 0;
    } catch (e) {
