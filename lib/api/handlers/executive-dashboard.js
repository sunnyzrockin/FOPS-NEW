...
export async function handleTwelveMonthTrend(request) {
  try {
    ...
    const { data: reports, error } = await db()
      .from('shift_reports')
      .select('date, site_id, total_sales, total_revenue, fuel_sales, shop_sales, total_litres')
    ...
    for (const r of reports || []) {
      ...
      if (!b) continue;
      const rev = Number(r.total_revenue) || Number(r.total_sales) || 0;
      b.revenue += rev;
      b.fuelSales += Number(r.fuel_sales) || 0;
      b.shopSales += Number(r.shop_sales) || 0;
      b.totalLitres += Number(r.total_litres) || 0;
      b.reportCount += 1;
    }
  ...
  try {
    ...
    async function sumRange(from, to) {
      const { data, error } = await db()
        .from('shift_reports')
        .select('total_sales, total_revenue, fuel_sales, shop_sales, total_litres')
        .in('site_id', scope.siteIds)
      ...
      for (const r of data || []) {
        acc.revenue += Number(r.total_revenue) || Number(r.total_sales) || 0;
        acc.fuelSales += Number(r.fuel_sales) || 0;
        acc.shopSales += Number(r.shop_sales) || 0;
        acc.totalLitres += Number(r.total_litres) || 0;
        acc.reports += 1;
      }
      return acc;
    }
...
export async function handleTopPerformers(request) {
  try {
    ...
    let q = db()
      .from('shift_reports')
      .select('site_id, total_sales, total_revenue, fuel_sales, shop_sales, total_litres')
      .in('site_id', scope.siteIds);
    ...
    for (const r of reports || []) {
      ...
      s.revenue += Number(r.total_revenue) || Number(r.total_sales) || 0;
      s.fuelSales += Number(r.fuel_sales) || 0;
      s.shopSales += Number(r.shop_sales) || 0;
      s.totalLitres += Number(r.total_litres) || 0;
      s.reportCount += 1;
      agg.set(r.site_id, s);
    }
