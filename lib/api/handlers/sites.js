...
export async function handleCreateSite(request) {
  try {
    ...
    // owner's new active site count. Non-blocking — failure logs but
    // doesn't fail the site creation (a subsequent sync-quantity call
    // will catch up).
    try {
      const { syncQuantityForOwner } = await import('@/lib/billing-sync');
      syncQuantityForOwner(auth.user.id).catch((e) =>
        console.warn('[sites] post-create quantity sync failed:', e?.message)
      );
    } catch (_) { /* lib not available in pre-billing-v2 deploys */ }
...
export async function handleUpdateSite(request, siteId) {
  try {
    ...
    let before = null;
    try { const { data } = await admin.from('sites').select('*').eq('id', siteId).single(); before = data; } catch {}
...
export async function handleDeleteSite(request, siteId) {
  try {
    ...
    // credit to next invoice). Non-blocking.
    try {
      const { syncQuantityForOwner } = await import('@/lib/billing-sync');
      syncQuantityForOwner(me.id).catch((e) =>
        console.warn('[sites] post-delete quantity sync failed:', e?.message)
      );
    } catch (_) { /* lib not available in pre-billing-v2 deploys */ }
