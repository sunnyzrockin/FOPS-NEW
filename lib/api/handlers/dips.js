...
export async function handleCreateDip(request) {
  try {
    ...
    const anyDelivery = [deliveries_ulp_litres, deliveries_diesel_litres, deliveries_premium_litres]
      .some((v) => Number(v) > 0);
    // Custom grades count as "something was recorded" too.
    const anyCustom = !!(custom_values && typeof custom_values === 'object'
      && Object.values(custom_values).some((v) => v && (v.level != null || Number(v.delivery) > 0)));
    ...
    // Sanitize custom_values: coerce every level/delivery to numbers,
    // drop empty entries entirely.
    let cleanCustom = {};
    if (custom_values && typeof custom_values === 'object' && !Array.isArray(custom_values)) {
      for (const [k, raw] of Object.entries(custom_values)) {
        if (!raw || typeof raw !== 'object') continue;
        const level = toNum(raw.level);
        const delivery = toNumZero(raw.delivery);
        // Skip if BOTH are empty (level null + delivery 0) — operator added
        // the field but staff didn't fill anything for it this shift.
    ...
    logAuditAsync({
      request, actor: currentUser, action: 'insert',
      tableName: 'dip_readings', recordId: data.id, siteId: data.site_id, after: data,
    });
...
export async function handleUpdateDip(id, request) {
  try {
    ...
    // custom_values is handled separately because it's an object.
    if ('custom_values' in body) {
      ...
      if (cv && typeof cv === 'object' && !Array.isArray(cv)) {
        for (const [k, raw] of Object.entries(cv)) {
          if (!raw || typeof raw !== 'object') continue;
          const level = raw.level === '' || raw.level == null ? null : Number(raw.level);
          const delivery = raw.delivery === '' || raw.delivery == null ? 0 : Number(raw.delivery);
          if (level == null && delivery === 0) continue;
          clean[k] = { level, delivery };
        }
      }
      patch.custom_values = clean;
    }
    ...
    logAuditAsync({
      request, actor: currentUser, action: 'update',
      tableName: 'dip_readings', recordId: id, siteId: data.site_id, before: existing, after: data,
    });
...
export async function handleDeleteDip(id, request) {
  try {
    ...
    logAuditAsync({
      request, actor: currentUser, action: 'delete',
      tableName: 'dip_readings', recordId: id, siteId: existing.site_id, before: existing,
    });
