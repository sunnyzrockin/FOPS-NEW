...
// any-authenticated. Every operation must reference a site the caller
// owns/operates/works at. For row-targeted writes (PUT/DELETE by id), we
// load the row first to look up its site_id and check that the caller has
// access.
// -------------------------------------------------------------------------
async function siteAccessDenied(user, siteId) {
...
async function lookupRowSiteId(table, id) {
  const { data } = await (supabaseAdmin || supabase)
    .from(table).select('site_id').eq('id', id).maybeSingle();
  return data?.site_id || null;
}

async function lookupCompetitorSiteId(competitorId) {
  const { data } = await (supabaseAdmin || supabase)
    .from('site_competitors').select('site_id').eq('id', competitorId).maybeSingle();
  return data?.site_id || null;
}
...
export async function handleGetSiteCompetitors(request) {
  try {
    ...
    const denied = await siteAccessDenied(auth.user, siteId);
...
export async function handleCreateSiteCompetitor(request) {
  try {
    ...
    if (!body?.site_id) {
      return NextResponse.json({ error: 'site_id is required' }, { status: 400, headers: corsHeaders });
    }
    const denied = await siteAccessDenied(auth.user, body.site_id);
    if (denied) return denied;
...
export async function handleUpdateSiteCompetitor(competitorId, request) {
  try {
    ...
    const targetSiteId = await lookupRowSiteId('site_competitors', competitorId);
    if (!targetSiteId) {
      return NextResponse.json({ error: 'Competitor not found' }, { status: 404, headers: corsHeaders });
    }
    const denied = await siteAccessDenied(auth.user, targetSiteId);
    if (denied) return denied;
    if (updates?.site_id && updates.site_id !== targetSiteId) {
      const denyNew = await siteAccessDenied(auth.user, updates.site_id);
...
export async function handleGetFuelPriceEntries(request) {
  try {
    ...
    // Always scope by site ownership: if a siteId is provided it must be
    // accessible; if not, fall back to the caller's full allowed set.
    const allowedSiteIds = await getAllowedSiteIds(auth.user);
    if (siteId) {
    ...
    else query = query.in('site_id', allowedSiteIds.length ? allowedSiteIds : ['__none__']);
...
export async function handleUpdateFuelPriceEntry(entryId, request) {
  try {
    ...
    const updates = await request.json();
    const targetSiteId = await lookupRowSiteId('fuel_price_entries', entryId);
    if (!targetSiteId) {
...
export async function handleGetCompetitorPrices(request) {
  try {
    ...
    // If a siteId is provided, gate it. If a competitorId is provided,
    // resolve its site_id and gate that. Otherwise, scope the result set
    // to the caller's allowed sites.
    const allowedSiteIds = await getAllowedSiteIds(auth.user);
    if (siteId && !allowedSiteIds.includes(siteId)) {
      return NextResponse.json({ error: 'You are not authorised to access this site.' }, { status: 403, headers: corsHeaders });
    }
    if (competitorId) {
      const compSiteId = await lookupCompetitorSiteId(competitorId);
      if (compSiteId && !allowedSiteIds.includes(compSiteId)) {
        return NextResponse.json({ error: 'You are not authorised to access this competitor.' }, { status: 403, headers: corsHeaders });
      }
    }
    ...
    if (!siteId && !competitorId) {
      query = query.in('site_id', allowedSiteIds.length ? allowedSiteIds : ['__none__']);
    }
    if (date) query = query.eq('date', date);
...
export async function handleUpdateCompetitorPrice(priceId, request) {
  try {
    ...
    const updates = await request.json();
    const targetSiteId = await lookupRowSiteId('competitor_fuel_prices', priceId);
    if (!targetSiteId) {
      return NextResponse.json({ error: 'Competitor price not found' }, { status: 404, headers: corsHeaders });
    }
    const denied = await siteAccessDenied(auth.user, targetSiteId);
    if (denied) return denied;
    if (updates?.site_id && updates.site_id !== targetSiteId) {
...
export async function handleGetFuelPriceComparison(request) {
  try {
    ...
    // Scope: drop any requested site the caller can't see. If nothing
    // remains return an empty array (legitimate state for a user with no
    // assignments to the requested sites).
    const allowedSet = new Set(await getAllowedSiteIds(auth.user));
    siteIdList = siteIdList.filter((id) => allowedSet.has(id));
    if (!siteIdList.length) {
