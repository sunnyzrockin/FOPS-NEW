 ...
 */
/**
 * Sum litres sold for a given grade matcher from a list of reports.
 * The matcher can be a RegExp (legacy callers) or a (key) => boolean
 * function (new callers — used by the custom-grade path to do strict
 * prefix matching and avoid the diesel/pre_diesel ambiguity).
 * Falls back to null if no per-grade litres are found.
 */
function sumMeteredSalesForGrade(reports, matcher) {
  const test = typeof matcher === 'function' ? matcher : (k) => matcher.test(k);
  let total = 0;
  let found = 0;
...
export async function handleWetstockReconciliation(request) {
  try {
    ...
    const [sitesRows, tolRows, dipsAll, reportsAll, fieldConfigsRows] = await Promise.all([
      ...
      // Bug #4: load each site's configured ENABLED dip fields so we
      // know whether the site uses the legacy fixed grades
      // (ULP/Diesel/Premium) or has its own custom grade dictionary. We
      // must NOT emit fixed-grade rows for sites that have any enabled
      // custom dip fields (would render 3 noise 'no_dips' rows above the
      // real grades — KINGSTHORPE showed 7).
      db.from('site_field_configs')
        .select('site_id, key, category, is_enabled')
        .in('site_id', siteIds)
        .eq('category', 'dip')
        .eq('is_enabled', true)
    ...
    for (const site of sites) {
      ...
      for (const ck of customGradeKeys) {
        ...
        const bookMovement = r2(opening - closing + deliveries);
        // Bug #4 follow-up: build a STRICT matcher for the custom grade
        // key. The previous regex `(?:^|[_\s-])${ck}(?:[_\s-]|$)` was
        // greedy — for ck="diesel" it ALSO matched "pre_diesel_litres",
        // double-counting metered sales (KINGSTHORPE diesel = 22400 L
        // when the right answer was 15400 L because pre_diesel was
        // bleeding in). Now we accept only the keys that start with
        // EXACTLY `${ck}_` (or equal `${ck}`) and are litre-shaped — no
        // ambiguity. Each shift_report's custom_values keys are the
        // canonical source so exact-prefix is the right discipline.
        const ckLower = ck.toLowerCase();
        const matcher = (k) => {
          const kLower = String(k).toLowerCase();
          // exact equality (rare — e.g. ck="ulp" and key "ulp" with a numeric value)
          if (kLower === ckLower) return true;
          // ck followed by underscore: "diesel_litres" but NOT "pre_diesel_litres"
          if (kLower.startsWith(`${ckLower}_`)) return true;
          return false;
        };
        const { litres: meteredSales, count: meteredCount } =
          sumMeteredSalesForGrade(siteReports, matcher);
