 ...
 *
 * DEMO BYPASS: is_demo users skip all subscription checks. They are
 * exempt from billing entirely — read-only enforcement happens at the
 * write layer (assertNotDemo + verifyAuth) instead. Without this
 * short-circuit, a brand-new demo tenant with no subscription row hits
 * the `no_subscription` branch and gets gated behind a checkout wall,
 * which contradicts the "Explore the demo — no billable tenant" spec.
 */
export async function getSubscriptionForUser(user) {
  if (user?.is_demo) {
    return { status: null, subscription: null, locked: false, lockReason: null, demo: true };
  }
