...
/**
 * Supabase Seeder for FOPS
 * Uses supabaseAdmin (service role) to bypass RLS and create auth users
 */
...
export async function seedAssignments() {
  ...
  const { error: opError } = await supabase
    ...
    .upsert(operatorAssignments, { 
      onConflict: 'operator_user_id,site_id',
  ...
  const { error: staffError } = await supabase
    ...
    .upsert(staffAssignments, { 
      onConflict: 'staff_user_id,site_id',
      ignoreDuplicates: false 
    });
